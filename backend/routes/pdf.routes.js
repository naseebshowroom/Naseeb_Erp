...
const buildData = (installment) => {
  ...
  const mapG = (g) => g
    ? { fullName: g.fullName || '', fatherName: g.fatherName || '', phone: g.phone || '' }
    : {};
  return {
    ...
    itemName: (
      installment.category === 'other'
        ? (installment.customItemName || installment.customCategory || 'Other Samaan')
        : `${installment.brand || ''} ${installment.model || ''}`
    ).trim(),
...
/**
 * Sends a PDF buffer as a downloadable response.
 */
const sendPDF = (res, buf, filename) => {
  res.set({
    ...
    // Prevent Railway / CDN from caching PDFs between users
...
/**
 * Shared error handler for all PDF routes.
 * Returns { success: false, message } so the frontend toast shows the real reason.
 */
const pdfError = (res, err, route) => {
  console.error(`[PDF] /${route} failed:`, err.message);
...
/**
 * Fetches a fully-populated installment document.
 */
...
router.post('/electronics-agreement', protect, async (req, res) => {
  try {
    ...
    if (!installmentId) return res.status(400).json({ success: false, message: 'installmentId is required' });
    ...
    pdfError(res, err, 'electronics-agreement');
  }
});
...
router.post('/motorcycle-agreement', protect, async (req, res) => {
  try {
    ...
    pdfError(res, err, 'motorcycle-agreement');
  }
});
...
router.post('/car-agreement', protect, async (req, res) => {
  try {
    ...
    pdfError(res, err, 'car-agreement');
  }
});

// ─── Sale Receipt (legacy / installment sale) ──────────────────────────────────
router.post('/sale-receipt', protect, async (req, res) => {
  try {
    ...
    const data = {
      ...
      cashPrice,
      registrationFee,
      totalAmount:   cashPrice + registrationFee,
      bikeCompany:   inst.brand  || '',
      bikeModel:     inst.model  || '',
      bikeColor:     inst.color  || '',
...
router.post('/cash-sale-receipt', protect, async (req, res) => {
  try {
    ...
    pdfError(res, err, 'cash-sale-receipt');
  }
});
...
router.post('/customer-statement', protect, async (req, res) => {
  try {
    ...
    const totalMissed = schedule.filter(s => s.status === 'missed').reduce((sum) => sum + perQ, 0);
    const totalPending= schedule.filter(s => s.status === 'pending').reduce((sum) => sum + perQ, 0);

    // All actual payment records for this installment (includes bulk/FIFO)
    ...
    const data = {
      ...
      advanceAmount:        inst.advanceAmount   || 0,
      perInstallmentAmount: perQ,
    ...
    pdfError(res, err, 'customer-statement');
  }
});
...
router.post('/distributor-letter', protect, async (req, res) => {
  try {
    ...
    if (!distributorId) return res.status(400).json({ success: false, message: 'distributorId is required' });
    ...
    if (!item) return res.status(404).json({ success: false, message: 'Item not found in distributor record' });
    ...
    pdfError(res, err, 'distributor-letter');
  }
});

// ─── Payment Receipt (GET + POST) ─────────────────────────────────────────────
const handleReceiptPDF = async (req, res) => {
  try {
    ...
    // Resolve due date from schedule entry if not stored on payment directly
    ...
    const receiptData = {
      ...
      // Snapshot fields first (stored at payment time), fall back to populated refs
      customerName:     payment.customerName      || payment.customer?.fullName  || 'N/A',
      customerPhone:    payment.customerPhone     || payment.customer?.phone     || 'N/A',
      customerCnic:     payment.customer?.cnic    || 'N/A',
      khataNumber:      payment.khataNumber       || payment.installment?.khataNumber || 'N/A',
      ...
      remainingBalance: payment.remainingAfterPayment !== undefined
        ? payment.remainingAfterPayment
      ...
      shortfall:        payment.shortfall !== undefined
        ? payment.shortfall
        : (payment.installment?.perInstallmentAmount
            ? Math.max(0, payment.installment.perInstallmentAmount - payment.amount)
            : 0),
    ...
    pdfError(res, err, `receipt/${req.params.paymentId}`);
  }
};
